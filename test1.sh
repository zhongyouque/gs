...
model_base_path="/home/zy/Project/test/gaussian-splatting/outputs/bicycle_test_V24"  # 模型路径的基础目录
    ...
    for x in "${x_list[@]}"; do
        for y in "${y_list[@]}"; do
            for small in "${small_list[@]}"; do
                # 为每组参数创建唯一的模型路径
                model_path="$model_base_path/dp_${dp}_x_${x}_y_${y}_small_${small}"
