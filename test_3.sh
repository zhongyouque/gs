...
# 执行训练命令，并保存输出
CUDA_VISIBLE_DEVICES=2 python train.py -s $dataset_path -m $model_path --clone 'v0' --split 'v0' --dp 0.005 --ip '127.0.0.17' --eval 2>&1 | tee -a $log_file
...
CUDA_VISIBLE_DEVICES=2 python render.py -m $model_path --skip_train 2>&1 | tee -a $log_file
...
for iteration in 30000 25000 20000; do
    ...
    CUDA_VISIBLE_DEVICES=2 python render.py -m $model_path --iteration "$iteration" --skip_train 2>&1 | tee -a $log_file
done
