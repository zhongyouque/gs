...
# 执行训练命令，并保存输出
CUDA_VISIBLE_DEVICES=5 python train.py -s $dataset_path -m $model_path --clone 'v0' --split 'v0' --dp 0.005 --ip '12.0.0.5' --eval 2>&1 | tee -a $log_file
...
CUDA_VISIBLE_DEVICES=5 python render.py -m $model_path --skip_train 2>&1 | tee -a $log_file
...
CUDA_VISIBLE_DEVICES=5 python metrics.py -m $model_path 2>&1 | tee -a $log_file
